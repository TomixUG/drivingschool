...
class WindowsVideoPlayer extends VideoPlayerPlatform {
  ...
  Future<int?> create(DataSource dataSource) async {
    if (dataSource.sourceType == DataSourceType.file) {
      ...
      final ByteData assetByteData = await rootBundle.load(dataSource.asset!);
      final Uint8List assetBytes = assetByteData.buffer.asUint8List();

      String randomName = "${Random().nextDouble() * 10000}.mp4";

      final File destinationFile = File(randomName);
      await destinationFile.writeAsBytes(assetBytes);

      var controller = WinVideoPlayerController.file(File(randomName));
      await controller.initialize();

      await File(randomName).delete();
      return controller.textureId_ > 0 ? controller.textureId_ : null;
    } else {
